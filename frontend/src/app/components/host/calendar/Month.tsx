...
const Month: React.FC<MonthProps> = ({ year, monthIndex, monthName, guestData, hostBlockedData }) => {
  ...
  const renderMonth = () => {
    ...
    for (let i = 0; i < startOffset; i++) {
      days.push(<div key={`empty-${i}`} className="h-24 sm:h-32 md:h-40"></div>);
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const isBlocked = guestForDay !== undefined || isDateBeforeToday(currentDate) || isDateHostBlocked(currentDate);
