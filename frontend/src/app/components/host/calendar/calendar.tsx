...
function Calendar({hostBlockedData, guestData}: {hostBlockedData: HostBlockedData[], guestData: GuestData[]}) 
{
  const today = new Date();
  const currentMonth = today.getMonth();
  const currentYear = today.getFullYear();

  const getNextMonths = (startMonth: number, startYear: number, count: number) => {
    const months = [];
  ...
  return (
    <div className='container mx-auto px-2 sm:px-4 box-border w-4/5'>
      <div className="space-y-8">
        {nextMonths.map(({ monthIndex, year }, index) => (
          <Month 
            key={`${year}-${monthIndex}`} 
            year={year} 
